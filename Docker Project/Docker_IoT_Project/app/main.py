...
# MQTT Configuration
MQTT_BROKER = os.environ.get('MQTT_BROKER', 'mqtt-broker')  # Docker'dan gelen environment variable'ları kullan
...
influx_host = os.environ.get('INFLUXDB_HOST', 'influxdb')  # Docker'dan gelen environment variable'ları kullan
influx_port = 8086
...
try:
    ...
    print(f"InfluxDB'ye bağlanırken hata oluştu: {e}")
    # Bağlantı başarısız ise 10 saniye sonra tekrar dene
    time.sleep(10)
...
# MQTT broker'a bağlantı
try:
    client.connect(MQTT_BROKER, MQTT_PORT, 60)
except Exception as e:
    print(f"MQTT broker'a bağlanırken hata oluştu: {e}")
    # Bağlantı başarısız ise 10 saniye sonra tekrar dene
    time.sleep(5)
...
try:
    while True:
        ...
        # Check thresholds
        if (humidity1 and temperature1 and (temperature1 > 30 or humidity1 > 70)) or            (humidity2 and temperature2 and (temperature2 > 30 or humidity2 > 70)):
